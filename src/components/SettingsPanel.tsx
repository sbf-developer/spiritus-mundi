      ...
      <div className="p-3 space-y-5">
            ...
            {PROVIDERS.map((p) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-md transition-colors border ${
                  local.provider === p.id
                    ? 'bg-surface-active border-border-default text-text-primary'
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={handleSave} className="btn-primary flex-1 flex items-center justify-center gap-2">
            {saved && <Check size={13} strokeWidth={2} />}
            {saved ? 'Saved' : 'Save'}
          </button>
        </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-[11px] font-medium text-text-muted mb-1.5 block">{label}</label>
      {children}
    </div>
  )
}
